...
Light GetAdditionalLight(uint i, PreData preData, half4 shadowMask)
{
    Light light = GetAdditionalLight(i, preData.positionWS, shadowMask);
...
half4 CalculateShadowMask(PreData preData)
{
    // To ensure backward compatibility we have to avoid using shadowMask input, as it is not present in older shaders
    #if defined(SHADOWS_SHADOWMASK) && defined(LIGHTMAP_ON)
    half4 shadowMask = preData.shadowMask;
...
Light GetMainLight(PreData preData, half4 shadowMask)
{
    Light light = GetMainLight(preData.shadowCoord, preData.positionWS, shadowMask);
